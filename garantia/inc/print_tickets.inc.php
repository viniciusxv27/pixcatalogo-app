...
$duedate_search_type = hesk_GET('duedate_option', 'specific');
...
if ($duedate_search_type === 'specific' && $duedate_input && hesk_datepicker_get_date($duedate_input) !== false) {
    ...
    $sql .= " AND `due_date` BETWEEN NOW() AND (NOW() + INTERVAL ".intval($duedate_amount_value)." {$unit}) ";
    $sql_count .= " AND `due_date` BETWEEN NOW() AND (NOW() + INTERVAL ".intval($duedate_amount_value)." {$unit}) ";
}
